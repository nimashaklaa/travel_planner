...
from agents.planner_agent.utils.function import extract_before_parenthesis, get_valid_name_city
        ...
        if unit['transportation'] and unit['transportation'] != '-':
            ...
            if 'flight number' in value.lower():
                ...
                if len(flight.data[(flight.data['Flight Number'] == value.split('Flight Number: ')[1].split(',')[0]) & (
                        flight.data['OriginCityName'] == org_city) & (flight.data['DestCityName'] == dest_city)]) < 1:
                ...
                try:
                    ...
                    dest_city = '-'
                    print(
                        "The transportation {} in day {} can not be parsed and '-' will be used instead.".format(value,
                                                                                                                 i + 1))

                if 'self-driving' in value.lower():
                    if googleDistanceMatrix.run_for_evaluation(org_city, dest_city, mode='self-driving')[
                        'cost'] == None:
...
def is_valid_accommodaton(question, tested_data):
    ...
    for unit in consectutive_accommodation:
        ...
        if unit and unit[0] not in ['-', '']:
            ...
            # try:
            if len(accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (
                    accommodation.data['city'] == city)]) == 1 and unit[1] < accommodation.data[
                (accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (
                        accommodation.data['city'] == city)].iloc[0]['minimum nights']:
...
def is_valid_days(question, tested_data):
    lens = 0
    for i in range(min(question['days'], len(tested_data))):
        if tested_data[i] != {} and tested_data[i][
            'current_city'] != "You don't need to fill in the information for this or later days.":
            lens += 1
...
def is_not_absent(question, tested_data):
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if (unit['breakfast'] in ['', '-'] or unit['lunch'] in ['', '-'] or unit['dinner'] in ['',
                                                                                               '-']) and 'from ' not in \
                unit['current_city']:
