...
def judge_valid_room_type(info, annotation_data, accommodation_data_all):
    ...
    if annotation_data['local_constraint']['room type'] == 'not shared room' and \
            accommodation_data_filtered['room type'].values[0] == 'Shared room':
        return False
    # "shared room", "not shared room", "private room", "entire room"
    elif annotation_data['local_constraint']['room type'] == 'shared room' and \
            accommodation_data_filtered['room type'].values[0] != 'Shared room':
        return False

    elif annotation_data['local_constraint']['room type'] == 'private room' and \
            accommodation_data_filtered['room type'].values[0] != 'Private room':
        return False

    elif annotation_data['local_constraint']['room type'] == 'entire room' and \
            accommodation_data_filtered['room type'].values[0] != 'Entire home/apt':
        return False
...
def judge_valid_cuisine(info, annotation_data, restaurant_data_all, cuisine_set: set):
    if info != "-" and annotation_data['local_constraint']['cuisine'] is not None and annotation_data[
        'org'] not in info:
