...
from agents.planner_agent.utils.function import get_valid_name_city
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            res = accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (
                        accommodation.data['city'] == city)]
            if len(res) > 0:
                print(f"Checking house rule for accommodation: {name}, {city}")
                print(f"Expected house rule: {question['local_constraint']['house rule']}")
                print(f"Accommodation rules: {res['house_rules'].values[0]}")
                if question['local_constraint']['house rule'] == 'smoking' and 'No smoking' in str(
                        res['house_rules'].values[0]):
                    print("Failed: Smoking not allowed.")
                    return False, f"The house rule should be {question['local_constraint']['house rule']}."
                if question['local_constraint']['house rule'] == 'parties' and 'No parties' in str(
                        res['house_rules'].values[0]):
                    print("Failed: Parties not allowed.")
                    return False, f"The house rule should be {question['local_constraint']['house rule']}."
                if question['local_constraint']['house rule'] == 'children under 10' and 'No children under 10' in str(
                        res['house_rules'].values[0]):
                    print("Failed: No children under 10.")
                    return False, f"The house rule should be {question['local_constraint']['house rule']}."
                if question['local_constraint']['house rule'] == 'visitors' and 'No visitors' in str(
                        res['house_rules'].values[0]):
                    print("Failed: No visitors allowed.")
                    return False, f"The house rule should be {question['local_constraint']['house rule']}."
                if question['local_constraint']['house rule'] == 'pets' and 'No pets' in str(
                        res['house_rules'].values[0]):
                    print("Failed: Pets not allowed.")
                    return False, f"The house rule should be {question['local_constraint']['house rule']}."
        ...
        for i in range(min(question['days'], len(tested_data))):
            ...
            if unit['breakfast'] and unit['breakfast'] != '-':
                ...
                res = restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (
                            restaurants.data['City'] == city)]
                if len(res) > 0:
                    print(f"Checking breakfast cuisine: {name}, {city}")
                    print(f"Available cuisines: {res.iloc[0]['Cuisines']}")
            ...
            if unit['lunch'] and unit['lunch'] != '-':
                ...
                res = restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (
                            restaurants.data['City'] == city)]
                if len(res) > 0:
                    print(f"Checking lunch cuisine: {name}, {city}")
                    print(f"Available cuisines: {res.iloc[0]['Cuisines']}")
                    for cuisine in question['local_constraint']['cuisine']:
                        if cuisine in res.iloc[0]['Cuisines']:
                            cuisine_set.add(cuisine)
            ...
            if unit['dinner'] and unit['dinner'] != '-':
                ...
                res = restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (
                            restaurants.data['City'] == city)]
                if len(res) > 0:
                    print(f"Checking dinner cuisine: {name}, {city}")
                    print(f"Available cuisines: {res.iloc[0]['Cuisines']}")
                    for cuisine in question['local_constraint']['cuisine']:
                        if cuisine in res.iloc[0]['Cuisines']:
                            cuisine_set.add(cuisine)
...
def is_valid_transportation(question, tested_data):
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if unit['transportation'] and unit['transportation'] != '-':
            value = unit['transportation']
            print(f"Checking transportation: {value}")
            print(f"Expected transportation constraint: {question['local_constraint']['transportation']}")
            if question['local_constraint']['transportation'] == 'no flight' and 'Flight' in value:
                print("Failed: Flight found when 'no flight' is expected.")
                ...
                print("Failed: Self-driving found when 'no self-driving' is expected.")
                return False, f"The transportation should not be {question['local_constraint']['transportation']}."
    ...
    for i in range(min(question['days'], len(tested_data))):
        ...
        if unit['accommodation'] and unit['accommodation'] != '-':
            ...
            res = accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (
                        accommodation.data['city'] == city)]
            if len(res) > 0:
                print(f"Checking room type for accommodation: {name}, {city}")
                print(f"Expected room type: {question['local_constraint']['room type']}")
                print(f"Accommodation room type: {res['room type'].values[0]}")

                if question['local_constraint']['room type'] == 'not shared room' and res['room type'].values[
                    0] == 'Shared room':
                    print("Failed: Shared room found when 'not shared room' is expected.")
                ...
                elif question['local_constraint']['room type'] == 'shared room' and res['room type'].values[
                    0] != 'Shared room':
                    print("Failed: Non-shared room found when 'shared room' is expected.")
                    return False, f"The room type should be {question['local_constraint']['room type']}."

                elif question['local_constraint']['room type'] == 'private room' and res['room type'].values[
                    0] != 'Private room':
                    print("Failed: Room type mismatch for 'private room'.")
                    return False, f"The room type should be {question['local_constraint']['room type']}."

                elif question['local_constraint']['room type'] == 'entire room' and res['room type'].values[
                    0] != 'Entire home/apt':
                    print("Failed: Room type mismatch for 'entire room'.")
                    return False, f"The room type should be {question['local_constraint']['room type']}."
...
def evaluation(query_data, tested_data):
    ...
    print("Evaluation complete.", return_info)
    return return_info


def boolean_evaluation(query_data, tested_data):
    print("Starting boolean evaluation...")
    return_info = {'valid_cuisine': is_valid_cuisine(query_data, tested_data),
                   ...
                   'valid_cost': (bool(get_total_cost(query_data, tested_data) <= query_data['budget']), None)}
    for key in return_info:
        print(f"Checking: {key}, Result: {return_info[key]}")
        if not return_info[key][0]:
            print(f"Constraint failed: {key}")
            print(key)
            return False
