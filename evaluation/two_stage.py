...
sys.path.append(os.path.abspath(os.path.join(os.getcwd(), "../agents/planner_agent")))
...
class ReactAgent:
    ...
    def load_city(self, city_set_path: str) -> List[str]:
        city_set_path = 'D:/semester 7/FYP/agents/planner_agent/database/background/citySet.txt'
        city_set = []
...
if __name__ == '__main__':
    ...
    output_dir = "../agents/tool_agent/"

    print(os.getenv('GOOGLE_API_KEY'))
    print("arges :", set_type)
    if set_type == 'validation':
        query_data_list = load_dataset('osunlp/TravelPlanner', 'validation')['validation']
        df = query_data_list.to_pandas()
        # Select the last 20 rows of the DataFrame
        query_data_list = df[["query", "reference_information"]].iloc[:2]
        print("if...........", query_data_list)
    elif set_type == 'test':
        query_data_list = load_dataset('osunlp/TravelPlanner', 'test')['test']
        print("else.............", query_data_list)
    numbers = [i for i in range(1, len(query_data_list) + 1)]
    agent = ReactAgent(None, tools=tools_list, max_steps=30, react_llm_name=model_name,
                       planner_llm_name= model_name)
    with get_openai_callback() as cb:

        for number in tqdm(range(1, len(query_data_list) + 1)):
            query = query_data_list.iloc[number - 1]['query']
            # check if the directory exists
            if not os.path.exists(os.path.join('./validation')):
                os.makedirs(os.path.join('./validation'))
            if not os.path.exists(os.path.join(f'./validation/generated_plan_{number}.json')):
                result = [{}]
            else:
                result = json.load(
                    open(os.path.join(f'./validation/generated_plan_{number}.json')))
            ...
            if planner_results == 'Max Token Length Exceeded.':
                result[-1][f'{model_name}_two-stage_results_logs'] = scratchpad
                result[-1][f'{model_name}_two-stage_results'] = 'Max Token Length Exceeded.'
                action_log[-1]['state'] = 'Max Token Length of Planner Exceeded.'
                result[-1][f'{model_name}_two-stage_action_logs'] = action_log
            else:
                result[-1][f'{model_name}_two-stage_results_logs'] = scratchpad
                result[-1][f'{model_name}_two-stage_results'] = planner_results
                result[-1][f'{model_name}_two-stage_action_logs'] = action_log

            # write to json file
            with open(os.path.join(f'./validation/generated_plan_{number}.json'), 'w') as f:
                json.dump(result, f, indent=4)

            print(f"Stored result in {f'./validation/generated_plan_{number}.json'}")
